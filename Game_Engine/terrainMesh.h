...
public:
    TerrainMesh( const std::string &file, float radus, float xOffset, float yOffset, float zOffset, float scale, bool generateBuffers, const std::string &position, int level, float &lastX, float &lastY, float &lastZ );
    virtual ~TerrainMesh();
